...
/**
 * Every icon's geometry, defined once.
 *
 * These used to live inside each icon component and be drawn inline at every usage. On a
 * twenty-four card catalogue that meant ninety-eight full `<svg>` elements carrying
 * seventy-six kilobytes of path data, and the same again inside the payload the page ships
 * for hydration. Now the shapes are emitted once by `<IconSprite />` and every usage is a
 * `<use>` reference.
 *
 * **What this actually bought, measured.** The catalogue page dropped from 496 KB to 469 KB
 * of markup and got about 14% cheaper to serve, because there is less to serialise and less
 * to compress. On the wire it is very slightly *worse*: gzip is extremely good at repeated
 * blocks, so ninety-eight identical SVGs compressed better than ninety-eight references plus
 * twenty-six definitions, and the compressed page grew by under two kilobytes. That trade is
 * taken deliberately. Compressed bytes are served from a CDN edge and cost us nothing, while
 * server CPU is the scarce resource on a small instance, and a browser parsing a hundred
 * `<use>` elements builds a smaller DOM than one parsing a hundred full drawings.
 *
 * Nothing about the artwork changed. The children below are the same elements that were in
 * each component, moved rather than redrawn.
 */
...
/**
 * The sprite. Rendered once per document, in the layout, before anything that uses an icon.
 *
 * `aria-hidden` and off-canvas rather than `display: none`, because a `<symbol>` inside a
 * hidden subtree is still referenceable but some browsers have historically been unreliable
 * about it, and because zero-size with `overflow: hidden` cannot affect layout either way.
 */
...
function Icon({ name, label, className = "" }: IconProps & { name: string }) {
  return (
    <svg
      viewBox={`0 0 ${SIZE} ${SIZE}`}
...
export function IconBox(props: IconProps) {
  return <Icon name="box" {...props} />;
}

/** Delivery. */
export function IconTruck(props: IconProps) {
  return <Icon name="truck" {...props} />;
}

/** Returns and exchanges. */
export function IconRotateLeft(props: IconProps) {
  return <Icon name="rotate-left" {...props} />;
}
