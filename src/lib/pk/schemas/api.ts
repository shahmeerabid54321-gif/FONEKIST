 ...
 *
 * Vendored from @pk/contracts `src/schemas/api.ts` by `pnpm sync:contracts`.
 * Edit it upstream in the WEBSITE DESIGN monorepo, then re-run the sync.
 */

import { z } from "zod";
import { PK_PROVINCES, addressSchema, cnicSchema, pkMobileSchema, pkrAmountSchema } from "./pakistan";
